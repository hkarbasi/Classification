...
class Terms(object):
    ...
    def construct_features(self):
        ...
        def label_weight_pos(count):
            return 1 if count > 0 else 0

        def label_weight_neg(count):
            return 0 if count > 0 else 1

        df_label = self.df_training.iloc[:, -1]

        retweeted_pos = df_label.apply(label_weight_pos)
        retweeted_neg = df_label.apply(label_weight_neg)

        def calc_prob(col, pos_retweeted_label, neg_retweeted_labels):
            #     print(type(col))
            #     print(retweeted_label)
            return dict(
                pos=sum(col * pos_retweeted_label) / sum(pos_retweeted_label),
                neg=sum(col * neg_retweeted_labels) / sum(neg_retweeted_labels)
            )



        prob_results=topic_training_df.apply(calc_prob,
                                             args=(retweeted_pos, retweeted_neg),
                                             axis=0
                                             )

        def calc_odd(message):
            indices = self.id2word.doc2idx(message)
            odd=1
            for index in indices:
                if index >= 0:
                    pos = prob_results[index]['pos'] if prob_results[index]['pos'] > 0 else len(topic_training_df)
                    neg = prob_results[index]['neg'] if prob_results[index]['neg'] > 0 else len(topic_training_df)
                    odd = odd * pos / neg
            return odd

        training_odds=self.df_training['final_text'].apply(calc_odd)


        topic_training_df = pd.concat([self.training_ids,
                                       training_odds
                                       ], ignore_index=True, axis=1)

        validation_odds=self.df_validation['final_text'].apply(calc_odd)

        topic_validation_df.reset_index(drop=True, inplace=True)
        topic_validation_df = pd.concat([self.validation_ids,
                                         validation_odds,
                                         ], ignore_index=True, axis=1)

        df_concat = pd.concat([topic_training_df,
                               topic_validation_df,
                               ], axis=0)
        df_concat.reset_index(drop=True, inplace=True)
        df_concat.columns=[self.c.id_name, 'terms']
...
def prep_input(df, context):
    ...
    return df[[context.id_name, 'final_text', 'label_col']]
...
def construct_features(_context, experiment, feature_name,
    ...
    instance = Terms(_context, experiment, feature_name,
                   df_training, df_validation,
                   training_ids, validation_ids)
